...
def rename_and_copy_files(source_dir, destination_pattern):
    ...
    for root, dirs, files in os.walk(source_dir):
        for filename in files:
            ...
            # Construct the full path of the destination file
            destination_path = os.path.join(
                 "/scratch/a/arisvoin/clarasun/SCanD_project_PNC/data/local/bids/sub-", 
                 destination_pattern, "/dwi_temp/", new_filename) # edited by adding forward slashes around dwi_temp
