...
SUB_SIZE=1 ## number of subjects to run is 1 because there are multiple tasks/runs that will run in parallel 
CORES=40
...
SUBJECTS=`sed -n -E "s/sub-(\S*)\>.*/\1/gp" ${BIDS_DIR}/participants_dwi.tsv | head -n ${bigger_bit} | tail -n ${SUB_SIZE}`
