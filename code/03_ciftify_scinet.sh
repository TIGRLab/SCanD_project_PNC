...
SUB_SIZE=1 ## number of subjects to run - try one at a time
CORES=40
...
SUBJECTS=`sed -n -E "s/sub-(\S*)\>.*/\1/gp" ${BIDS_DIR}/participants_original.tsv | head -n ${bigger_bit} | tail -n ${SUB_SIZE}`
